...
def read_poses_dict_6(fname):
    ...
    with open(fname) as f:
        ...
        for row in reader:
            ...
            pose_rot = txe.euler2mat(*pose[0:3]) # returns a matrix of rotations from Euler angles
            pose_trans = pose[3:]
            full_pose = np.eye(4) # diagonals have the value 1 and 0 otherwise
...
#         print('Poses Normalized! pose_format = {}'.format(pose_format))
        if pose_format == 'quat':
            all_poses[:, :3] -= poses_mean # Subtrating the mean from translations
            all_poses[:, :3] = np.divide(all_poses[:, :3], poses_std, where=poses_std!=0) # Dividing translation with std deviation! Hence normalized values!
        else: # 'full-mat'
...
class Apolloscape(Dataset):
    ...
    def __init__(self, root, road="", transform=None, record=None,
        ...
        """
            Args:
                ...
                stereo (bool): Return stereo pairs
        """
...
#                     print('create our splits')
        ...
        # Filter Train/Val
		# Checks where each image exists! Whether in the train or the validation dataset!
        if self.train is not None:
