...
class PoseNet1(torch.nn.Module):
    ...
    def __init__(self, feature_extractor, num_features=128, dropout=0.5,
        ...
        self.feature_extractor.classifier = torch.nn.Linear(512, num_features)
        # Translation
        ...
        # Initialization
        if self.pretrained:
            init_modules = [self.feature_extractor.classifier, self.fc_xyz, self.fc_quat]
        else:
            init_modules = self.modules()
          
		# For stereo implementation
...
#         x = self.feature_extractor(x)
        ...
        if type(x) is list:
            x_translations = [self.fc_xyz(xi) for xi in self.x_features]
            x_rotations = [self.fc_quat(xi) for xi in self.x_features]
            x_poses = [torch.cat((xt, xr), dim=1) for xt, xr in zip(x_translations, x_rotations)]  
        elif torch.is_tensor(x):
            x_translations = self.fc_xyz(self.x_features) 
            x_rotations = self.fc_quat(self.x_features)
            x_poses = torch.cat((x_translations, x_rotations), dim=1)
        return x_poses
